...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(label='用户名或密码', 
                                    widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'请输入用户名或密码'}))
...
class ChangeNicknameForm(forms.Form):
    ...
    def change_nickname_new(self):
        nickname_new = self.cleaned_data.get('nickname', '').strip()
        if nickname_new == '':
...
class ForgotPasswordForm(forms.Form):
    email = forms.EmailField(
        label='邮箱', 
        widget=forms.EmailInput(attrs={'class':'form-control', 'placeholder':'请输入绑定的的邮箱'})
    )
    verification_code =  forms.CharField(
        label='验证码', 
        required = False, 
    ...
    def clean_email(self):
        ...
        if not User.objects.filter(email=email).exists():
            return forms.ValidationError('邮箱不存在')
        return email
